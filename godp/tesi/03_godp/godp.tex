...
Queste funzioni condividono anche i passi che compiono per inserire le aggregazioni nella pipeline:
\begin{enumerate}
    \item creazione di un sotto-scopo da utilizzare per l'operazione
...
Questo modulo è responsabile della definizione delle entità coinvolte nel flusso di dati dell'applicazione e include anche un sotto-modulo di utilità dedicato a incapsulare la complessità del caricamento, formattazione e pulizia dei dati.
...
Il file \texttt{yaml\_config} implementa metodi che implementano la funzionalità di caricamento della configurazione YAML, che opera leggendo il file specificato da linea di comando ed effettua il parsing della struttura YAML. La libreria \texttt{goccy/go-yaml} fornisce un modo semplice per associare una serie di tipi struct innestati ai tag YAML corrispondenti. Utilizzando questa libreria in combinazione \texttt{go-playground/validator} è stato possibile definire appropriate regole di validazione dei valori di configurazione, la libreria associa poi appositi messaggi di errore ben formattati per indicare con precisione all'utente la posizione del campo mal specificato e il motivo dell'errore. Data la carenza di espressività di queste regole, che permettono di intervenire soltanto a livello del singolo campo, è stata definita una funzione che effettua controlli trasversali tra i campi, in particolare viene validato che per certi valori di un campo siano popolati specifici campi e assenti altri.
...
\begin{itemize}
    ...
    \item \mintinline{go}{var TypesMap map[string]string}: questa mappa è composta a partire dalla configurazione fornita, ha come chiave il nome di un campo e come valore una stringa che indica il tipo e
    \item \mintinline{go}{var IdFieldIndex int}: memorizza l'indice che indica la posizione della colonna scelta come identificatore di privacy dall'utente nell'array \texttt{Headers}.
\end{itemize}
...
Nella definizione della struttura \texttt{PipelineDp} è inclusa la struttura dati \texttt{Filters} come lista di \texttt{FilterType}, nella quale vengono caricate le informazioni sulla configurazione dei filtri da applicare al dataset, composto dal nome dell'attributo coinvolto, dall'operatore applicato e dal suo valore.
\begin{minted}[breaklines,bgcolor=lightgray,framesep=2mm,baselinestretch=1.2,fontsize=\footnotesize]{go}
...
\end{minted}
Gli operatori supportati sono uguaglianza, disuguaglianza e confronti di ordine, quando questi sono applicati nella pipeline DP tengono conto del tipo di dati dell'attributo facendo riferimento alla mappa dei tipi inclusa nel documento YAML.

L'applicazione dei filtri al dataset è realizzata tramite la struttura \texttt{FilterFn}, un \texttt{DoFn} di Apache Beam che implementa una logica congiuntiva (AND): quando sono specificati più filtri, un record viene incluso nel risultato soltanto se soddisfa \textit{tutti} i criteri definiti. Questa struttura viene utilizzata dalla funzione \texttt{ApplyFilters} che, ricevendo come parametri lo scope della pipeline, la \texttt{PCollection} contenente i dati, la lista dei filtri e la mappa dei tipi, applica l'operazione \texttt{ParDo} con il \texttt{DoFn} di filtraggio, restituendo una nuova \texttt{PCollection} contenente soltanto i record che soddisfano tutti i criteri. Se non è specificato alcun filtro, la \texttt{PCollection} originale viene restituita senza modifiche.
...
\begin{enumerate}
    \item \texttt{InitEnvironment}: funzione invocata prima di ogni run specializzata; qui viene configurata la pipeline, aggiunta a quest'ultima l'operazione di caricamento del dataset e raccolti i valori dei parametri da CLI;
...
DPYaml si articola in cinque sezioni:
\begin{itemize}
    \item configurazione
    ...
    \item operazioni
\end{itemize}
Tra questi, le sezioni di configurazione, budget di privacy e operazioni sono indispensabili al funzionamento dell'applicazione. La definizione dei tipi di dato si rende necessaria soltanto nei casi in cui si definiscano delle operazioni che richiedano gestione particolare del tipo di dato da elaborare, come ad esempio un'operazione che calcola la media di un valore aggregato su un campo chiave, al contrario di operazioni come il conteggio di elementi che appartengono a una categoria. La sezione dei filtri è opzionale: quando specificata richiede che tutte le colonne coinvolte in uno dei filtri siano anche incluse nella sezione dei tipi.
...
Questa sezione opzionale permette di definire una serie di filtri che restringono la porzione del dataset su cui vengono applicate le operazioni DP specificate.

Ogni filtro è caratterizzato da tre parametri:
\begin{itemize}
    \item \texttt{column}: il nome dell'attributo sul quale applicare il filtro
    \item \texttt{operator}: l'operatore di confronto da utilizzare, scelto tra \texttt{=}, \texttt{!=}, \texttt{<}, \texttt{>}, \texttt{<=} e \texttt{>=} e
    \item \texttt{value}: valore di riferimento
\end{itemize}

Quando più di un filtro è specificato, questi vengono composti in congiunzione logica, quindi soltanto i record che soddisfano tutti i criteri saranno inclusi nel dataset che verrà usato per le operazioni. Tutti gli attributi coinvolti devono avere il proprio tipo definito nella sezione \texttt{types} per permettere all'applicazione di valutare correttamente i confronti.
...
Il campo \texttt{importance} specifica un peso numerico che determina la porzione di budget di privacy allocata per ciascuna operazione. Il modulo \texttt{budget} utilizza questo valore per suddividere i parametri $\varepsilon$ e $\delta$ in modo proporzionale. Per ogni operazione viene assegnata una quota di budget calcolata come il rapporto tra il valore di \texttt{importance} dell'operazione e la somma del valore di \texttt{importance} di tutte le operazioni. In questo modo, operazioni con importanza elevata ricevono una quota maggiore del budget di privacy, risultando in un'aggregazione con una quantità di rumore minore, e quindi un risultato più preciso, a scapito delle altre operazioni.
...
\begin{itemize}
    ...
    \item \texttt{max\_contributions\_per\_category} indica il numero massimo di contribuzioni da un dato identificatore di privacy a una singola categoria (o chiave) sulla quale viene effettuata la media;
