...
Uno dei primi esempi di questi metodi è la tecnica \textit{Differentially Private Stochastic Gradient Descent} \cite{Abadi_2016dpsgd} che consiste in una variante dell'algoritmo SGD standard che prevede l'aggiunta di rumore accuratamente calibrato ai gradienti a ogni iterazione del processo di addestramento, questo approccio, combinato con l'imposizione di un limite superiore ai valori dei gradienti, permette di limitare l'influenza che un singolo campione può avere sui parametri del modello; questa tecnica è usata per addestrare modelli di deep learning.
